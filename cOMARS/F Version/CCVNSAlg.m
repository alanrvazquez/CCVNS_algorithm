...
% Define the vectors to calculate the frequencies of correlations.
if mod(n, 4) == 0
    ndivtwo = n/2;
    Lambda = 2*n - 4*(2:ndivtwo);
    Lambda = sort(Lambda);
elseif mod(n, 4) == 2
    nmsdivf = (n-6)/4;
    ll = 0:nmsdivf;
    Lambda = [8*ll, 2*n - 8*(ll+1)];
    Lambda = sort(Lambda);
end
Lambda(1) = 2;
nelLambda = size(Lambda, 2);
vecbigM = 10.^((0:(nelLambda-1))*3);
...
parfor jj = 1:maxiter
    ...
    [objvalue, deslower] = CCAlg(desupper, startdeslower, m, NSelTwo, None, N, vecbigM, nelLambda, Lambda); % Apply CC algorithm and compute the first (current) objective value
        ...
        switch k
            ...
            case 1 % N1. Sign-switch any column 
                ...
                for ll = 1:m
                    ...
                    % Apply CC Algorithm
                    [resvalue, DesLowNeigh]  = CCAlg(desupper, deslower, m, NSelTwo, None, N, vecbigM, nelLambda, Lambda);
            ...
            case 2 % N2. Swap any two columns 
                ...
                for ll = 1:None
                    ...
                    % Apply CC algorithm
                    [resvalue, DesLowNeigh]  = CCAlg(desupper, deslower(:, neworder), m, NSelTwo, None, N, vecbigM, nelLambda, Lambda);
    ...
    result(jj) = lcSOcorr( [desupper; deslower], NSelTwo, None, N, vecbigM, nelLambda, Lambda);
